...
export async function GET(req: NextRequest) {
  try {
    ...
    // If category include fails (e.g. deleted category), fallback without include
    try {
      const works = await prisma.work.findMany({
        orderBy: [{ featured: 'desc' }, { sortOrder: 'asc' }, { createdAt: 'desc' }],
      })
      ...
      console.error('GET /api/works fallback also failed:', fallbackError?.message || fallbackError)
      return NextResponse.json({ error: fallbackError?.message || String(fallbackError), stack: fallbackError?.stack?.slice(0, 500) || '' }, { status: 500 })
    }
  }
}
...
export async function DELETE(req: NextRequest) {
  ...
  return NextResponse.json({ error: 'id or ids required' }, { status: 500 })
}
